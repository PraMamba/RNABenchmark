...
class SupervisedDataset(Dataset):
	...
	def __init__(self, data_path: str, args, tokenizer: transformers.PreTrainedTokenizer, kmer: int=-1):
		...
		self.weight_mask = torch.ones(self.attention_mask.shape)
...
def calculate_metric_with_sklearn(logits: np.ndarray, labels: np.ndarray):
	...
	return \
		{
			"Acceptor_TopK-Acc" : metrics[1],
			"Donor_TopK-Acc": metrics[2],
			"Avg_TopK-Acc": np.mean(metrics[1:])
		}
...
def main():
	...
	if model_args.model_type == 'RNALM':
		...
		config = CustomBertConfig.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			token_type=model_args.token_type,
