...
from model.dnabert2.bert_layers import DNABERT2ForCRISPROffTarget
from model.genalm.modeling_genalm import GENALMForCRISPROffTarget
from model.caduceus.modeling_caduceus import CaduceusForCRISPROffTarget
from model.nucleotide_transformer.modeling_nt import EsmForCRISPROffTarget
from model.evo.modeling_hyena import StripedHyenaForCRISPROffTarget
from model.hyenadna.modeling_hyena import HyenaDNAForCRISPROffTarget
...
def calculate_metric_with_sklearn(logits: np.ndarray, labels: np.ndarray):
    ...
    return {"MSE": sklearn.metrics.mean_squared_error(labels, logits), "Spearman" : scipy.stats.spearmanr(labels, logits)[0]}
...
def main():
	...
	if model_args.model_type == 'RNALM':
		...
		logger.info(f'Loading {model_args.model_type}')
		model = DNABERT2ForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
		...
		logger.info(f'Loading {model_args.model_type}')
		model = GENALMForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
		...
		logger.info(f'Loading {model_args.model_type}')
		model = CaduceusForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
		...
		logger.info(f'Loading {model_args.model_type}')
		model = EsmForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
		...
		logger.info(f'Loading {model_args.model_type}')
		model = StripedHyenaForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
		...
		logger.info(f'Loading {model_args.model_type}')
		model = HyenaDNAForCRISPROffTarget.from_pretrained(
			model_args.model_name_or_path,
			cache_dir=model_args.cache_dir,
			num_labels=train_dataset.num_labels,
			problem_type="regression",
