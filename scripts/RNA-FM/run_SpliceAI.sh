...
token_type='single'
model_type='RNA-FM'
model_name_or_path="/pri_exthome/Mamba/Project/GRE_EMB/Evaluate/BEACON/Pretrain/RNA-FM"
...
"
    ...
    --metric_for_best_model eval_Avg_TopK-Acc \
...
if [[ "$train_type" == "torchrun" ]]; then
    ...
    NUM_PROC=8
    CUDA_VISIBLE_DEVICES=${GPU_DEVICES} torchrun --nproc_per_node=${NUM_PROC} downstream/train_spliceai.py $common_args >> "${log_file}" 2>&1
else
    ...
    deepspeed --include ${GPU_DEVICES} --master_port ${MASTER_PORT} downstream/train_spliceai.py --deepspeed ${DEEPSPEED_CONFIG} $common_args >> "${log_file}" 2>&1
fi
