...
output_dir="/pri_exthome/Mamba/Project/GRE_EMB/Evaluate/BEACON/FineTurn/${task}/${model_type}/gena-lm-bert-base-t2t"
...
if [[ "$train_type" == "torchrun" ]]; then
    ...
    NUM_PROC=8
    CUDA_VISIBLE_DEVICES=${GPU_DEVICES} torchrun --nproc_per_node=${NUM_PROC} downstream/train_mean_ribosome_loading.py $common_args >> "${log_file}" 2>&1
else
    ...
    deepspeed --include ${GPU_DEVICES} --master_port ${MASTER_PORT} downstream/train_mean_ribosome_loading.py --deepspeed ${DEEPSPEED_CONFIG} $common_args >> "${log_file}" 2>&1
fi
