...
token_type='single'
model_type='GENA-LM'
model_name_or_path="/pri_exthome/Mamba/Project/GRE_EMB/Evaluate/BEACON/Pretrain/GENA-LM/gena-lm-bert-base-t2t"
...
output_dir="/pri_exthome/Mamba/Project/GRE_EMB/Evaluate/BEACON/FineTurn/Modif/${model_type}/gena-lm-bert-base-t2t"
...
"
    ...
    --metric_for_best_model eval_mean_auc \
...
if [[ "$train_type" == "torchrun" ]]; then
    ...
    NUM_PROC=8
    CUDA_VISIBLE_DEVICES=${GPU_DEVICES} torchrun --nproc_per_node=${NUM_PROC} downstream/train_modification.py $common_args >> "${log_file}" 2>&1
else
    ...
    deepspeed --include ${GPU_DEVICES} --master_port ${MASTER_PORT} downstream/train_modification.py --deepspeed ${DEEPSPEED_CONFIG} $common_args >> "${log_file}" 2>&1
fi
